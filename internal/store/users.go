...
func (s *UserStore) GetByID(ctx context.Context, id int64) (*User, error) {
	...
	err := s.db.QueryRowContext(ctx, query, id).Scan(&user.ID, &user.Email, &user.Username, &user.Password.hash, &user.CreatedAt,
		&user.Role.ID,
		&user.Role.Name,
		&user.Role.Level,
		&user.Role.Desc,
...
func (s *UserStore) GetByEmail(ctx context.Context, email string) (*User, error) {
	...
	err := s.db.QueryRowContext(ctx, query, email).Scan(&user.ID, &user.Email, &user.Username, &user.Password.hash, &user.CreatedAt)
